...
lines = codecs.open('app_v2.py', encoding='utf-8').readlines()

# Find start and end of tab_calc
start_idx = -1
end_idx = -1
for i, line in enumerate(lines):
    if line.startswith('with tab_calc:'):
        start_idx = i
    if line.startswith('with tab_redev:'):
        end_idx = i
        break

tab_calc_code = """with tab_calc:
...
new_lines = lines[:start_idx] + [tab_calc_code + "\n\n"] + lines[end_idx - 1:]

codecs.open('app_v2.py', 'w', encoding='utf-8').writelines(new_lines)
codecs.open('app.py', 'w', encoding='utf-8').writelines(new_lines)
codecs.open('app_restored.py', 'w', encoding='utf-8').writelines(new_lines)
